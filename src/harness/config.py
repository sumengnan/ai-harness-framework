...
class HarnessConfig(BaseSettings):
    ...
    max_steps: int = 100
    # 循环/停滞检测的窗口，一个开关管**两道**判据；<2 关闭两者。
    # 两道都是「先注入纠偏提示给一次机会，之后仍不变才中止」，防模型白跑到 max_steps。
    #
    # 1. **签名**：连续 N 步发起完全相同的工具调用（同名+同参）—— 原地打转。
    # 2. **结果**：同一个工具连续 N 次返回逐字相同的结果，**不管参数变没变** ——
    #    换着花样撞同一堵墙。第 1 道漏的正是它：实测一次真跑里模型九次改同一个
    #    函数、九次跑测试拿回逐字相同的失败输出，九次的参数各不相同，签名检测
    #    一次都没响，一路烧到预算耗尽。这一类更贵，因为它看起来像在推进。
